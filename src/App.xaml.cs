...
public partial class App : Application
{
    ...
    protected override async void OnStartup(StartupEventArgs e)
    {
        ...
        if (e.Args.Contains("--self-test")) { try { if (e.Args.Contains("--recording")) await SelfTest.RunRecording(); else await SelfTest.Run(e.Args.Contains("--integration")); Shutdown(0); } catch (Exception ex) { System.IO.File.WriteAllText(System.IO.Path.Combine(AppContext.BaseDirectory, e.Args.Contains("--recording") ? "recording-test-results.txt" : "test-results.txt"), ex.ToString()); Shutdown(1); } return; }
